 ...
 * 
 * First step in signup flow where users choose between Mentor and Mentee roles.
 * Features interactive role cards with detailed benefits and visual styling.
 * 
 * Key Features:
 * - Visually distinct cards for each role with gradient backgrounds
 * - Detailed benefit lists for each role
 * - Selected state visualization
 * - Continue button enabled only when role is selected
 * 
 * State Management:
 * - Tracks currently selected role
 * - Manages continue button state
 * 
 * Props:
 * @prop {function} onRoleSelect - Callback when user selects a role and continues
 *   Signature: (role: {id: string, title: string, ...}) => void
 * 
 * UI Components:
 * - Role cards with icons and gradients
 * - Benefit items with custom SVG icons
